...
def split_into_blocs(binary_text):
...
def decrypt_text(binary_blocks, mapping):
    binary_to_symbol = {key: value for key, value in mapping.items()}
    decrypted = "".join(binary_to_symbol.get(block, '') for block in binary_blocks)
    return decrypted
...
def decrypt(C):
    ...
    binary_blocks = split_into_blocs(C)
    dictionary = gen_key(symboles)

    mapping = {value: key for key, value in dictionary.items()}
    M = decrypt_text(binary_blocks, mapping)
